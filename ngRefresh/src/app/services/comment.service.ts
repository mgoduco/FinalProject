...
export class CommentService {
    ...
    return this.http.get<Comment[]>(`${this.url}/recipes/${id}/comments`).pipe(
      catchError((err: any) => {
        ...
        return throwError(
          () =>
            new Error(
              'CommentService.index(): error retrieving Comment: ' + err
            )
        );
      })
    );
  ...
  getCommentByRecipe(recipe: Recipe, comment: Comment): Observable<Comment> {
    return this.http
      .get<Comment>(
        `${this.url}/recipes/${recipe.id}/comments/${comment.id}`,
        this.getHttpOptions()
      )
      .pipe(
        catchError((err: any) => {
  ...
  getCommentsByUser(user: User): Observable<Comment[]> {
    return this.http
      .get<Comment[]>(
        `${this.url}/comments/user/${user.username}`,
        this.getHttpOptions()
      )
      .pipe(
        catchError((err: any) => {
      ...
      .pipe(
        catchError((err: any) => {
          ...
          return throwError(
            () =>
              new Error(
                'CommentService.delete(): error deleting Comment: ' + err
              )
          );
        })
      );
