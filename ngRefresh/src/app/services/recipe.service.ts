...
export class RecipeService {
  ...
  recipesByUsername(user: User): Observable<Recipe[]> {
    return this.http.get<Recipe[]>(this.url + "/u/" +  user.username, this.getHttpOptions()).pipe(
      catchError((err: any) => {
        console.log(err);
        return throwError(
  ...
  recipesByNameandIngredient(keyword: string): Observable<Recipe[]> {
    return this.http.get<Recipe[]>(this.url + "/search/" + keyword, this.getHttpOptions()).pipe(
      catchError((err: any) => {
        console.log(err);
        return throwError(
          () => new Error('RecipeService.index(): error retrieving Recipes: ' + err)
  ...
  delete(id: number): Observable<Recipe> {
    return this.http.delete<Recipe>(`${this.url}/${id}`, this.getHttpOptions()).pipe(
      catchError((err: any) => {
        console.error(err);
        return throwError(
